...
class CreateService extends Command
{
    ...
    protected $description = 'Create services in PennyLane from Holded data';
    ...
    {
        try {
            $response = $this->holdedClient->get('https://api.holded.com/api/invoicing/v1/services');
            ...
            foreach ($products as $product) {
                if ($this->shouldProcessProduct($product)) {
                    ...
                    $pennylaneData = $this->createServicePennylane($product); // Correctly use the actual method
    ...
    private function createServicePennylane($data)
    {
        try {
